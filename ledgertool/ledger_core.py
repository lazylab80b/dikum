...
def run_command(argv: List[str]) -> int:
    ...
    try:
        if cmd == "index":
            if not rest:
                print("ERROR " + json.dumps({"code":"bad_args","hint":"index <dir> [-o ledger.csv]"}))
                return 1
            directory = rest[0]
            out = None
            if "-o" in rest:
            ...
            if "-l" not in rest:
                print("ERROR " + json.dumps({"code":"bad_args","hint":"audit -l ledger.csv [-d images_dir]"}))
                return 1
            li = rest.index("-l")
            ...
            if "-l" not in rest or "-o" not in rest:
                print("ERROR " + json.dumps({"code":"bad_args","hint":"export -l ledger.csv -o out.zip [-d images_dir]"}))
                return 1
            li = rest.index("-l"); oi = rest.index("-o")
            ledger = rest[li+1] if li+1 < len(rest) else None
            ...
            if ledger is None or not os.path.isfile(ledger):
                ...
                return 1
            if outzip is None:
                print("ERROR " + json.dumps({"code":"bad_args","hint":"export -l ledger.csv -o out.zip"}))
                return 1
            if "-d" in rest:
                di = rest.index("-d")
                images_dir = rest[di+1] if di+1 < len(rest) else None
